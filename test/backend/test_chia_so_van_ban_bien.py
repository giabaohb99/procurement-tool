...
def _people(db, code: str, company_id: int, dept_id: int = 0, is_active: bool = True):
...
def cty_khac(db, seed):
    other = Company(code="CTX", name="Công ty X", issue_code="CTX", level=2, is_active=True)
    db.add(other)
    db.flush()
    db.commit()
    return other
...
def test_khong_co_grant_nao_van_thay_so_duoc_chia(db, seed, cty_khac):
    ...
    """
    nv, tk = _people(db, "SO_KHONG_QUYEN", cty_khac.id)
    so = _so(db, company_id=seed.company_id, manager_ids=[seed.emp_tp_id],
             viewer_ids=[nv.id])
    db.commit()
...
def test_so_NGUNG_DUNG_van_hien_cho_nguoi_duoc_chia(db, seed, cty_khac):
    ...
    """
    nv, tk = _people(db, "SO_NGUNG", cty_khac.id)
    so = _so(db, company_id=seed.company_id, manager_ids=[seed.emp_tp_id],
             viewer_ids=[nv.id])
...
def test_so_CHUA_GAN_PHAP_NHAN_khong_no_va_van_chia_duoc(db, cty_khac):
    ...
    """
    nv, tk = _people(db, "SO_MO_COI", cty_khac.id)
...
def test_nhan_su_da_NGHI_thi_khong_con_thay(db, seed, cty_khac, grant_role):
    ...
    """
    nv, tk = _people(db, "SO_NGHI", cty_khac.id)
    so = _so(db, company_id=seed.company_id, manager_ids=[seed.emp_tp_id],
             viewer_ids=[nv.id])
    nv.is_active = False
    db.commit()
...
def test_go_MOT_nguoi_khong_lam_nguoi_kia_mat_quyen(db, seed, cty_khac):
    ...
    """
    nv1, tk1 = _people(db, "SO_N1", cty_khac.id)
    nv2, tk2 = _people(db, "SO_N2", cty_khac.id)
    so = _so(db, company_id=seed.company_id, manager_ids=[seed.emp_tp_id],
...
def test_chia_NHIEU_so_thi_thay_du_ca_ba_loai(db, seed, cty_khac):
    ...
    nv, tk = _people(db, "SO_BA_LOAI", cty_khac.id)
    code = []
    for kind in (1, 2, 3):
    ...
    assert _danh_sach(db, tk) == sorted(code)


def test_khong_chia_so_nao_thi_danh_sach_RONG_chu_khong_no(db, cty_khac):
    _, tk = _people(db, "SO_TRANG", cty_khac.id)
    db.commit()
    assert _danh_sach(db, tk) == []
...
def test_pham_vi_ALL_thi_dieu_kien_la_None_va_thay_het(db, seed, cty_khac, grant_role):
    ...
    assert book_service.book_view_condition(tk, get_perm_profile(db, tk)) is None
    assert _danh_sach(db, tk) == sorted([so_a.code, so_b.code])


# ── Đường mở MỘT quyển ───────────────────────────────────────────────────────
def test_id_khong_ton_tai_ra_404_chu_khong_no(db, cty_khac):
    _, tk = _people(db, "SO_404", cty_khac.id)
    db.commit()

    with pytest.raises(HTTPException) as error:
        book_service.viewable_book_or_404(db, 999999, tk, get_perm_profile(db, tk))
...
def test_so_da_xoa_thi_dong_thanh_vien_khong_keo_ai_vao_duoc(db, seed, cty_khac):
    ...
    nv, tk = _people(db, "SO_DA_XOA", cty_khac.id)
    so = _so(db, company_id=seed.company_id, manager_ids=[seed.emp_tp_id],
             viewer_ids=[nv.id])
    book_id = so.id
    ...
    remaining = db.query(DocumentBookMember).filter(
...
def test_nguoi_duoc_chia_mo_duoc_BO_DEM_cua_so(db, seed, cty_khac):
    ...
    nv, tk = _people(db, "SO_DEM", cty_khac.id)
    so = _so(db, company_id=seed.company_id, manager_ids=[seed.emp_tp_id],
             viewer_ids=[nv.id])
    db.commit()

    profile = get_perm_profile(db, tk)
    assert book_service.viewable_book_or_404(db, so.id, tk, profile).id == so.id
...
def test_nguoi_QUAN_LY_so_thay_so_du_khong_co_vai_tro(db, seed, cty_khac):
    ...
    nv, tk = _people(db, "SO_QL", cty_khac.id)
