...
def test_khai_ke_hoach_duoc_tren_van_ban_con_nhap(db, entity_context):
    """Đây là toàn bộ lý do kế hoạch tồn tại — clone thật thì không làm được."""
    from datetime import date

    rows = clone_service.set_plan(
        db, entity_context["nhap"], [entity_context["a"].id, entity_context["b"].id],
        date(2026, 9, 30), "Sửa hạn mức ở Điều 5", ACTOR,
    )

    assert {row.company_id for row in rows} == {entity_context["a"].id, entity_context["b"].id}
    assert all(row.due_date == date(2026, 9, 30) for row in rows)
    #  Khai kế hoạch KHÔNG được sinh văn bản nào.
    assert clone_service.clones_of(db, entity_context["nhap"].id) == []


def test_khong_khai_ke_hoach_ve_chinh_phap_nhan_ban_hanh(db, entity_context):
    with pytest.raises(HTTPException) as error:
        clone_service.set_plan(
            db, entity_context["nhap"], [entity_context["seed"].company_id], None, "", ACTOR)
    assert "chính pháp nhân" in error.value.detail


def test_khong_khai_ke_hoach_cho_phap_nhan_khong_ton_tai(db, entity_context):
    with pytest.raises(HTTPException) as error:
        clone_service.set_plan(db, entity_context["nhap"], [999999], None, "", ACTOR)
    assert "không tồn tại" in error.value.detail


# ── 2 · ghi đè, không cộng dồn ──────────────────────────────────────────────
def test_ghi_de_ke_hoach_chu_khong_cong_don(db, entity_context):
    ...
    """
    clone_service.set_plan(
        db, entity_context["nhap"], [entity_context["a"].id, entity_context["b"].id], None, "", ACTOR)

    rows = clone_service.set_plan(db, entity_context["nhap"], [entity_context["b"].id], None, "", ACTOR)

    assert [row.company_id for row in rows] == [entity_context["b"].id]


def test_ke_hoach_rong_la_huy_ke_hoach(db, entity_context):
    clone_service.set_plan(db, entity_context["nhap"], [entity_context["a"].id], None, "", ACTOR)

    assert clone_service.set_plan(db, entity_context["nhap"], [], None, "", ACTOR) == []


# ── 3 · clone xong thì gỡ khỏi kế hoạch ─────────────────────────────────────
def test_clone_xong_thi_go_khoi_ke_hoach(db, entity_context):
    """Để nguyên thì pháp nhân đó vừa "dự kiến" vừa "đã có" — đếm hai lần."""
    clone_service.set_plan(
        db, entity_context["nhap"], [entity_context["a"].id, entity_context["b"].id], None, "", ACTOR)
    origin = _ban_hanh(db, entity_context["nhap"])

    clone_service.create_clones(db, origin, [entity_context["a"].id], None, "", ACTOR)

    remaining = clone_service.plan_for(db, origin.id)
    assert [row.company_id for row in remaining] == [entity_context["b"].id]


def test_clone_lay_han_va_ghi_chu_tu_ke_hoach_khi_khong_khai_de(db, entity_context):
    """Hạn khai từ lúc tạo phải theo sang bản clone, không thì khai để làm gì."""
    from datetime import date

    clone_service.set_plan(
        db, entity_context["nhap"], [entity_context["a"].id],
        date(2026, 9, 30), "Giữ nguyên Điều 1–4", ACTOR,
    )
    origin = _ban_hanh(db, entity_context["nhap"])

    clone = clone_service.create_clones(db, origin, [entity_context["a"].id], None, "", ACTOR)[0]

    assert clone.clone_due_date == date(2026, 9, 30)
    assert clone.clone_note == "Giữ nguyên Điều 1–4"


def test_han_khai_luc_bam_clone_thang_han_trong_ke_hoach(db, entity_context):
    from datetime import date

    clone_service.set_plan(
        db, entity_context["nhap"], [entity_context["a"].id], date(2026, 9, 30), "cũ", ACTOR)
    origin = _ban_hanh(db, entity_context["nhap"])

    clone = clone_service.create_clones(
        db, origin, [entity_context["a"].id], date(2026, 12, 31), "mới", ACTOR)[0]
...
def test_khong_xep_lai_phap_nhan_da_co_ban_clone(db, entity_context):
    """Xếp lại thì lần bấm clone sau đâm vào UNIQUE(source, company)."""
    origin = _ban_hanh(db, entity_context["nhap"])
    clone_service.create_clones(db, origin, [entity_context["a"].id], None, "", ACTOR)

    rows = clone_service.set_plan(
        db, origin, [entity_context["a"].id, entity_context["b"].id], None, "", ACTOR)

    assert [row.company_id for row in rows] == [entity_context["b"].id]


# ── Danh sách chờ phải nói rõ nơi nào đã khai ───────────────────────────────
def test_danh_sach_cho_danh_dau_phap_nhan_da_khai_ke_hoach(db, entity_context):
    """Cờ này là thứ giúp hộp thoại tick sẵn — mất nó thì phải chọn lại từ đầu."""
    clone_service.set_plan(db, entity_context["nhap"], [entity_context["a"].id], None, "", ACTOR)
    origin = _ban_hanh(db, entity_context["nhap"])

    theo_ten = {row["company_name"]: row for row in clone_service.pending_companies(db, origin)}
