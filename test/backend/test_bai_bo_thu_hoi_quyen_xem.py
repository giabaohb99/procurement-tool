...
def test_nguoi_tao_van_xem_duoc(doc_da_bai_bo):
    assert revoke_access.con_xem_duoc(doc_da_bai_bo, _user(NGUOI_TAO), _profile()) is True
...
def test_nguoi_bai_bo_van_xem_duoc(doc_da_bai_bo):
    ...
    assert revoke_access.con_xem_duoc(doc_da_bai_bo, _user(NGUOI_BAI_BO), _profile()) is True
...
def test_nguoi_chiu_trach_nhiem_noi_dung_van_xem_duoc(doc_da_bai_bo):
    ...
    nguoi = _user(NGUOI_NGOAI, employee_id=NHAN_SU_NCTN)
    assert revoke_access.con_xem_duoc(doc_da_bai_bo, nguoi, _profile()) is True
...
def test_nguoi_giu_so_van_xem_duoc(doc_da_bai_bo, scope):
    ...
    nguoi = _user(NGUOI_NGOAI, employee_id=999)
    assert revoke_access.con_xem_duoc(doc_da_bai_bo, nguoi, _profile(scope)) is True
...
def test_nguoi_ngoai_mat_quyen_xem(doc_da_bai_bo, scope):
    ...
    nguoi = _user(NGUOI_NGOAI, employee_id=999)
    assert revoke_access.con_xem_duoc(doc_da_bai_bo, nguoi, _profile(scope)) is False
...
def test_quyen_giu_so_phai_DUNG_HANH_DONG_doc(doc_da_bai_bo):
    ...
    assert revoke_access.con_xem_duoc(doc_da_bai_bo, nguoi, ho_so) is False
...
def test_van_ban_chua_bai_bo_thi_ai_cung_qua():
    ...
    assert revoke_access.con_xem_duoc(doc, _user(NGUOI_NGOAI, employee_id=999), _profile()) is True


def test_giu_so_thi_khong_can_loc_gi_them():
    """`None` = không cộng điều kiện nào vào truy vấn danh sách."""
    assert revoke_access.dieu_kien_loc(_user(NGUOI_NGOAI), _profile("all")) is None


def test_nguoi_thuong_thi_co_dieu_kien_loc():
    assert revoke_access.dieu_kien_loc(_user(NGUOI_NGOAI, 999), _profile("dept")) is not None
...
def test_bai_bo_theo_quan_he_ghi_dung_nguoi_bai_bo(db, seed):
    ...
    cu = Document(doc_type_id=doc_type.id, company_id=seed.company_id,
    ...
    moi = Document(doc_type_id=doc_type.id, company_id=seed.company_id,
    ...
    db.add_all([cu, moi])
    db.commit()
    db.add(DocumentLink(source_document_id=moi.id, target_document_id=cu.id,
                        relation=RELATION_REVOKE, created_by=NGUOI_BAN_HANH,
                        updated_by=NGUOI_BAN_HANH))
    db.commit()

    bi_bai_bo = apply_supersede(db, moi, NGUOI_BAN_HANH)
    db.commit()

    assert [d.id for d in bi_bai_bo] == [cu.id], "phải trả về văn bản bị bãi bỏ để chỗ gọi đi báo"
    assert cu.status == STATUS_REVOKED
    assert cu.expire_date is not None, "bãi bỏ phải đóng ngày hết hiệu lực, y như nút «Bãi bỏ»"
    assert cu.updated_by == NGUOI_BAN_HANH, "cột «người bãi bỏ» phải là người ban hành văn bản mới"

    #  Và hệ quả của nó: người sửa cuối cùng trước đó KHÔNG còn xem được.
    ho_so = _profile("own")
    assert revoke_access.con_xem_duoc(cu, _user(NGUOI_SUA_CUOI, 4), ho_so) is False
    assert revoke_access.con_xem_duoc(cu, _user(NGUOI_BAN_HANH, 2), ho_so) is True
