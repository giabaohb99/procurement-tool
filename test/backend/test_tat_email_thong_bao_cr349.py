...
#  Ba hộp thư TEST: đặt là mọi email bị đổi hướng ra đó (`_test_route_email`). Máy dev
#  thường có sẵn trong `.env`, nên không tắt thì bài kiểm dưới đọc ra địa chỉ của người
#  đang chạy test chứ không phải địa chỉ người nhận — hỏng vì môi trường, không phải vì mã.
_EMAIL_TEST_KEYS = ("EMAIL_TEST_OVERRIDE", "EMAIL_TEST_MANAGER", "EMAIL_TEST_STAFF")


@pytest.fixture
def bat_email_luong_duyet():
    """Bật công tắc toàn hệ + tắt đổi hướng hộp thư test, rồi trả lại giá trị cũ."""
    cu = getattr(settings, "EMAIL_WORKFLOW_ENABLED", False)
    cu_test = {k: getattr(settings, k, None) for k in _EMAIL_TEST_KEYS}
    settings.EMAIL_WORKFLOW_ENABLED = True
    for k, v in cu_test.items():
        if v is not None:
            setattr(settings, k, "")
    yield
