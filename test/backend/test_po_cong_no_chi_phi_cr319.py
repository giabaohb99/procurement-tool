...
def test_duyet_don_moi_dong_chi_phi_thanh_mot_khoan_no(db, seed):
    ...
    assert freight.due_date == "2026-10-15" and freight.status == "Chờ TT"
...
def test_tao_yctt_tach_moi_ncc_mot_phieu_va_chi_tien_tru_dung_khoan(db, seed):
    ...
    assert float(freight.paid_amount) == 1_080_000.0 and freight.status == "Đã TT"
    assert float(port.paid_amount) == 200_000.0 and port.status == "Đã TT"
