...
def align(db, seed):
    ...
    db.add_all([con_a, con_b, regulation])
    db.commit()

    origin = service.create_document(db, DocumentCreate(
        doc_type_id=regulation.id, company_id=me.id, department_id=seed.dept_id,
        owner_employee_id=seed.emp_req_id, title="Quy chế chi tiêu",
        content_html="<p>Nội dung</p>",
    ), ACTOR)
    db.commit()
    return {"goc": origin, "con_a": con_a, "con_b": con_b, "quy_che": regulation, "seed": seed}


def _tao_ban_rieng(db, align, company: Company, **thay_doi) -> Document:
    """Dựng thẳng bản clone: `clone_service.create` đòi bản gốc đã ban hành, mà
    bài này kiểm CÂY chứ không kiểm luật ban hành."""
    clone = Document(
        doc_type_id=align["quy_che"].id, company_id=company.id,
        department_id=align["seed"].dept_id, owner_employee_id=align["seed"].emp_req_id,
        title=align["goc"].title, source_document_id=align["goc"].id,
        status=align["goc"].status, origin=align["goc"].origin,
...
def test_cay_hien_ban_rieng_cua_tung_phap_nhan_con(db, align):
    _tao_ban_rieng(db, align, align["con_a"])
    _tao_ban_rieng(db, align, align["con_b"])

    cay = link_serializer.build_tree(db, align["goc"])
...
def test_chua_clone_thi_cay_van_rong(db, align):
    assert link_serializer.build_tree(db, align["goc"])["children"] == []


def test_moi_dong_kem_ten_phap_nhan_du_tieu_de_trung_nhau(db, align):
    ...
    cay = link_serializer.build_tree(db, align["goc"])
    title = {con["title"] for con in cay["children"]}
...
def test_ban_rieng_can_ra_lai_duoc_danh_dau_tren_cay(db, align):
    ...
    _tao_ban_rieng(db, align, align["con_a"], needs_review=True)

    con = link_serializer.build_tree(db, align["goc"])["children"][0]

    assert con["is_outdated"] is True
    assert con["needs_review"] is True


def test_ban_rieng_co_kem_quan_he_van_hien_la_ban_rieng(db, align):
    ...
    clone = _tao_ban_rieng(db, align, align["con_a"])
    db.add(DocumentLink(source_document_id=clone.id, target_document_id=align["goc"].id,
                        relation=RELATION_BASED_ON, is_system=True,
                        created_by=ACTOR, updated_by=ACTOR))
    db.commit()

    con = link_serializer.build_tree(db, align["goc"])["children"]
...
def test_ban_goc_cung_kem_ten_phap_nhan(db, align):
    assert link_serializer.build_tree(db, align["goc"])["company_name"] == "Cty Test"
