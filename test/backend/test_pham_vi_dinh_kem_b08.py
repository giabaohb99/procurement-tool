...
def test_dinh_kem_theo_dong_lan_nguoc_ra_phieu_chu_khong_nhan_id_dong(data):
    ...
    model, ids = asc.parent_records(data.db, "survey_request_line", data.dong_b.id)
    assert model is SurveyRequest
    assert ids == [data.ycks_b.id]

    model, ids = asc.parent_records(data.db, "delivery", data.giao_b.id)
    assert model is PurchaseOrder
    assert ids == [data.po_b.id]
...
# ── Tầng lọc ────────────────────────────────────────────────────────────────────
def test_dinh_kem_hop_dong_phap_nhan_khac_bi_chan(data, nguoi_dung):
    ...
    with pytest.raises(HTTPException) as e:
        _doc(data.db, user, "contract", data.hd_b.id)
    assert e.value.status_code == 403


def test_dinh_kem_hop_dong_phap_nhan_minh_van_mo_duoc(data, nguoi_dung):
    user = nguoi_dung(_profile(["contract"], scope="company", company_id=CTY_A))
    assert _doc(data.db, user, "contract", data.hd_a.id)


def test_dinh_kem_theo_dong_ngoai_pham_vi_bi_chan(data, nguoi_dung):
    ...
    with pytest.raises(HTTPException) as e:
        _doc(data.db, user, "survey_request_line", data.dong_b.id)
    assert e.value.status_code == 403
    assert _doc(data.db, user, "survey_request_line", data.dong_a.id)


def test_gan_them_tep_vao_phieu_ngoai_pham_vi_cung_bi_chan(data, nguoi_dung):
    ...
    with pytest.raises(HTTPException) as e:
        _doc(data.db, user, "delivery", data.giao_b.id, mode="manage")
    assert e.value.status_code == 403


def test_pham_vi_tat_ca_van_thay_het(data, nguoi_dung):
    ...
    assert _doc(data.db, user, "contract", data.hd_b.id)
    assert _doc(data.db, user, "survey_request_line", data.dong_b.id)


def test_quyen_vai_tro_van_chan_truoc_nhu_cu(data, nguoi_dung):
    ...
    with pytest.raises(HTTPException) as e:
        _doc(data.db, user, "contract", data.hd_a.id)
    assert e.value.status_code == 403


def test_chung_tu_cha_khong_con_thi_404_chu_khong_phai_403(data, nguoi_dung):
    ...
    with pytest.raises(HTTPException) as e:
        _doc(data.db, user, "survey_request_line", 999999)
    assert e.value.status_code == 404


# ── Không chặn nhầm ─────────────────────────────────────────────────────────────
def test_tai_tep_tam_chua_gan_vao_dau_thi_khong_soi_pham_vi(data, nguoi_dung):
    """`POST /upload-file` gọi `_check` KHÔNG kèm `entity_id` — chưa có bản ghi nào để soi."""
    user = nguoi_dung(_profile(["contract"], scope="company", company_id=CTY_A))
    assert ac._check(data.db, user, "contract", "manage")


def test_o_khong_co_chung_tu_cha_thi_khong_soi_pham_vi(data, nguoi_dung):
    ...
    assert ac._check(data.db, user, "comment", "manage", 12345)


# ── Đường lùi theo chuỗi chứng từ ───────────────────────────────────────────────
def test_tep_pyc_ngoai_pham_vi_nhung_thuoc_don_minh_xem_duoc_thi_tai_duoc(data, nguoi_dung):
    ...
    with pytest.raises(HTTPException):
        _doc(data.db, user, "purchase_request", data.pyc_a.id)
    #  …nhưng đơn PO-A thì mở được, nên tệp PYC-A trong chuỗi vẫn tải lẻ được.
    data.po_a.created_by = USER_ID
    data.db.commit()
    assert asc.reachable_from_scoped_po(data.db, user, "purchase_request", data.pyc_a.id)


def test_duong_lui_lan_hai_chang_tu_ycks_qua_phieu_khao_sat(data, nguoi_dung):
    """YCKS → PKS → ĐMH: thiếu chặng giữa là dòng YCKS trong chuỗi bấm vào 403."""
    user = nguoi_dung(_profile(["purchase_order", "survey_request"],
                               scope="own", company_id=CTY_A))
    data.po_a.created_by = USER_ID
    data.db.commit()
    assert asc.reachable_from_scoped_po(data.db, user, "survey_request_line", data.dong_a.id)


def test_duong_lui_khong_mo_cho_tep_khong_thuoc_don_nao_minh_xem_duoc(data, nguoi_dung):
    user = nguoi_dung(_profile(["purchase_order", "purchase_request"],
                               scope="company", company_id=CTY_A))
    assert not asc.reachable_from_scoped_po(data.db, user, "purchase_request", data.pyc_b.id)


def test_duong_lui_khong_ap_cho_hop_dong(data, nguoi_dung):
    ...
    assert not asc.reachable_from_scoped_po(data.db, user, "contract", data.hd_b.id)
...
def test_tai_mot_tep_hop_dong_ngoai_pham_vi_van_403_du_co_duong_lui(data, nguoi_dung, monkeypatch):
    ...
    data.db.add(f)
    data.db.flush()
    lk = FileLink(file_id=f.id, entity="contract", entity_id=data.hd_b.id,
                  doc_type="", sort_order=0)
    data.db.add(lk)
    data.db.commit()

    with pytest.raises(HTTPException) as e:
        ac.download_one(lk.id, data.db, user)
    assert e.value.status_code == 403
...
def test_tai_mot_tep_trong_pham_vi_van_chay(data, nguoi_dung, monkeypatch):
    ...
    data.db.add(f)
    data.db.flush()
    lk = FileLink(file_id=f.id, entity="contract", entity_id=data.hd_a.id,
                  doc_type="", sort_order=0)
    data.db.add(lk)
    data.db.commit()

    assert ac.download_one(lk.id, data.db, user).body == b"noi dung"
...
def test_chuoi_chung_tu_khong_con_no_importerror(data, nguoi_dung):
    ...
    po, groups = ac._resolve_chain(data.db, user, "purchase_order", data.po_a.id)
    assert po.code == "PO-A"
    #  Chuỗi phải lần đủ ba chặng theo mã, không dừng ở mỗi cái đơn.
    source = {ent for ent, _, _, _ in groups}
    assert {"purchase_order", "delivery", "purchase_request", "survey",
            "survey_request"} <= source
