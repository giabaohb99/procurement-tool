...
def _tra(db, code) -> SecurityLevel:
...
# ── Thêm ──────────────────────────────────────────────────────────────────────
def test_mot_thang_khong_duoc_co_hai_bac_trung_so(db, mat):
...
# ── Xóa ───────────────────────────────────────────────────────────────────────
def test_con_van_ban_dang_o_bac_do_thi_khong_xoa_duoc(db, mat):
    _van_ban(db, secrecy_level=3)

    with pytest.raises(HTTPException) as e:
        before_delete(db, mat)
...
def test_do_khan_dem_dung_cot_urgency_chu_khong_phai_secrecy(db):
    ...
    with pytest.raises(HTTPException) as e:
        before_delete(db, khan)
    assert "1 văn bản" in e.value.detail


def test_loai_van_ban_dang_lay_lam_mac_dinh_thi_khong_xoa_duoc(db, mat):
...
def test_khong_con_ai_dung_thi_xoa_duoc(db, mat):
    _van_ban(db, secrecy_level=2)          # văn bản ở bậc KHÁC
    before_delete(db, mat)                  # không ném gì cả


# ── Xóa: điều kiện luồng duyệt (chỗ nguy hiểm nhất) ───────────────────────────
def _luong(db, dieu_kien, ten="Luồng ban hành"):
    row = ApprovalFlow(name=ten, entity="document", condition=json.dumps(dieu_kien),
                       created_by=1, updated_by=1)
    db.add(row)
    db.flush()
    return row
...
def test_luong_duyet_dang_tro_toi_thi_khong_xoa_duoc(db, mat):
...
def test_bat_duoc_ca_dieu_kien_long_nhieu_tang(db, mat):
...
def test_bat_duoc_khi_so_nam_trong_danh_sach_cua_phep_in(db, mat):
...
def test_khong_bat_nham_bac_khac_cung_thang(db, mat):
...
def test_khong_bat_nham_so_dai_hon_chua_chu_so_do(db, mat):
...
def test_khong_bat_nham_truong_khac_cung_con_so(db, mat):
...
def test_bat_ca_dieu_kien_khai_o_tung_nut_khong_chi_o_luong(db, mat):
...
def test_bac_khong_co_trong_danh_muc_thi_bi_tu_choi(db, mat):
    with pytest.raises(HTTPException) as e:
        ensure_valid(db, KIND_CONFIDENTIAL, 9)
    assert "không có bậc 9" in e.value.detail


def test_bac_da_ngung_dung_thi_khong_chon_moi_duoc(db, mat):
    mat.is_active = False
    db.flush()
...
# ── Nhãn cho Excel ────────────────────────────────────────────────────────────
def test_doi_ten_bac_thi_file_excel_in_theo_ten_moi(db, mat):
    """Trước đây `export.py` giữ bản chép riêng của bảy cái nhãn, nên đổi tên
    trên màn hình xong file Excel vẫn in chữ cũ."""
    mat.name = "Mật — hạn chế"
    db.flush()

    nhan_mat, _ = label_maps(db)
    assert label(nhan_mat, 3) == "Mật — hạn chế"


def test_bac_da_ngung_dung_van_tra_ra_ten(db, mat):
    ...
    nhan_mat, _ = label_maps(db)
    assert label(nhan_mat, 3) == "Mật"


def test_so_la_thi_in_ra_chinh_con_so(db, mat):
    nhan_mat, _ = label_maps(db)
    assert label(nhan_mat, 9) == "9"


def test_tra_bac_theo_ma_chu_khong_theo_so(db, mat):
