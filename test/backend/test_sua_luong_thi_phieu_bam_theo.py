...
def _trinh(db, submitter, entity_id=DOC_ID):
...
def test_buoc_dang_cho_thi_viec_chuyen_sang_nguoi_moi(db, seed, person, entity_context):
    flow = _luong(db)
    b1 = _buoc(db, flow, 1, person["a"])

    instance = _trinh(db, person["nop"])
    assert [r.assignee_employee_id for r in _dang_cho(db, instance)] == [person["a"]]

    b1.approver_ref = str(person["b"])
    flow_sync_service.sync_after_step_edit(db, b1, ACTOR, entity_context)
    db.commit()

    assert [r.assignee_employee_id for r in _dang_cho(db, instance)] == [person["b"]], \
    ...
    old = [r for r in _viec(db, instance) if r.assignee_employee_id == person["a"]]
    assert [r.status for r in old] == [TASK_CANCELLED]


def test_nguoi_cu_khong_con_duyet_duoc_nua(db, seed, person, entity_context):
    flow = _luong(db)
    b1 = _buoc(db, flow, 1, person["a"])
    instance = _trinh(db, person["nop"])
...
def test_buoc_chua_toi_cung_di_theo_nguoi_moi(db, seed, person, entity_context):
    flow = _luong(db)
    _buoc(db, flow, 1, person["a"])
    b2 = _buoc(db, flow, 2, person["b"])

    instance = _trinh(db, person["nop"])
    b2.approver_ref = str(person["c"])
    flow_sync_service.sync_after_step_edit(db, b2, ACTOR, entity_context)
    db.commit()

    action_service.approve(db, instance, person["a"], ACTOR, {})
    assert [r.assignee_employee_id for r in _dang_cho(db, instance)] == [person["c"]]


# ── 3 · bước ĐÃ KÝ không đụng tới ───────────────────────────────────────────

def test_buoc_da_ky_giu_nguyen_chu_ky(db, seed, person, entity_context):
...
def test_sua_thanh_khong_con_ai_thi_phieu_ket_chu_khong_di_tiep(db, seed, person, entity_context):
...
def test_bo_nhanh_day_len_cap_tren_thi_phieu_ket(db, seed, person):
    ...
    assert instance_service.tasks_of_instance(db, instance.id) == []
...
def test_loai_chung_tu_chua_khai_ham_boi_canh_thi_tra_rong(db, seed):
    ...
    assert entity_hooks.entity_context(db, "khong_co_loai_nay", 1) == {}
