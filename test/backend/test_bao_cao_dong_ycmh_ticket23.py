...
def _make_item(db, pr, product_code: str, line_status: str = "no_po",
...
class TestBaoCaoDongYCMH:
    def test_tra_du_truong_gram_tu_specs_va_ten_nstm(self, db, seed, monkeypatch):
        ...
        assert r["amount"] == 10800.0 and r["line_status"] == "no_po"
    ...
    def test_loc_chua_dat_gop_hai_trang_thai(self, db, seed, monkeypatch):
        ...
        _make_item(db, pr, "SP-1", line_status="no_po")
        _make_item(db, pr, "SP-2", line_status="not_ordered")
        _make_item(db, pr, "SP-3", line_status="ordered")
        _make_item(db, pr, "SP-4", line_status="completed")
        db.commit()
        _see_all(monkeypatch)
        ...
        out2 = report_service.compute_pr_lines(db, USER, year="2026", line_status="ordered")
