...
def align(db, seed):
    ...
    person = {}
    for name in ("a", "b"):
        employee = Employee(code=f"BM_{name.upper()}", full_name=f"Người duyệt {name.upper()}",
    ...
    return {"doc": doc, "nguoi": person, "doc_type": doc_type, "seed": seed}
...
def test_co_tat_thi_gui_duyet_chay_y_nhu_cu(db, align):
    """Điều kiện số một của cam kết «không ảnh hưởng thứ đang chạy»."""
    doc = service.submit(db, align["doc"], ACTOR)
...
def test_co_tat_thi_van_duyet_bang_nut_cu(db, align):
    doc = service.submit(db, align["doc"], ACTOR)
...
def test_bat_co_ma_chua_khai_luong_thi_van_chay_duong_cu(db, align):
...
def test_bat_co_va_co_luong_thi_mo_phien_nhieu_buoc(db, align):
    _bat_co(db)
    _luong_hai_buoc(db, align["nguoi"])
    ...
    assert instance.entity_title == "Quy chế bảo mật"

    waiting = [row for row in instance_service.tasks_of_instance(db, instance.id)
                if row.status == TASK_PENDING]
    assert [row.assignee_employee_id for row in waiting] == [align["nguoi"]["a"]]


def test_ban_goc_va_clone_mo_hai_luong_rieng_theo_phap_nhan(db, align):
    ...
    child_approver = Employee(
        ...
        company_id=child.id, department_id=align["seed"].dept_id, is_active=True,
    )
    db.add(child_approver)
    db.commit()

    clone = service.create_document(db, DocumentCreate(
        doc_type_id=align["doc_type"].id,
        company_id=child.id,
        department_id=align["seed"].dept_id,
        owner_employee_id=align["seed"].emp_req_id,
        title="Quy chế bảo mật — bản công ty con",
        content_html="<p>Nội dung riêng.</p>",
    ), ACTOR)
    clone.source_document_id = align["doc"].id
    db.commit()

    _bat_co(db)
    root_flow = _luong_mot_buoc(
        db, "VB-ME", "Luồng pháp nhân gốc", align["seed"].company_id,
        align["nguoi"]["a"],
    )
    ...
    root_instance = instance_service.running_instance(db, ENTITY, align["doc"].id)
    child_instance = instance_service.running_instance(db, ENTITY, clone.id)
    ...
    assert [row.assignee_employee_id for row in instance_service.tasks_of_instance(
        db, root_instance.id) if row.status == TASK_PENDING] == [align["nguoi"]["a"]]
    assert [row.assignee_employee_id for row in instance_service.tasks_of_instance(
        db, child_instance.id) if row.status == TASK_PENDING] == [child_approver.id]


def test_clone_thieu_luong_rieng_bi_chan_truoc_khi_doi_trang_thai(db, align):
    child = Company(code="CON", name="Công ty con", issue_code="CON",
                    level=2, is_active=True)
    db.add(child)
    db.commit()
    ...
    assert instance_service.running_instance(db, ENTITY, clone.id) is None


def test_duyet_het_cac_buoc_thi_van_ban_duoc_BAN_HANH_that(db, align):
    ...
    action_service.approve(db, instance, align["nguoi"]["a"], ACTOR, {})
    db.refresh(doc)
    assert doc.status == STATUS_SUBMITTED, "Mới một chữ ký thì chưa ban hành"

    action_service.approve(db, instance, align["nguoi"]["b"], ACTOR, {})
...
def test_tu_choi_thi_van_ban_bi_khoa_o_da_tu_choi(db, align):
    ...
    action_service.reject(db, instance, align["nguoi"]["a"], ACTOR, "Thiếu căn cứ pháp lý")
...
def test_tra_lai_thi_van_ban_sang_tra_ve_va_gui_duyet_lai_duoc(db, align):
    ...
    action_service.send_back(db, instance, align["nguoi"]["a"], ACTOR, "Sửa lại mục 2", {})
    ...
    db.refresh(doc)
    assert doc.status == STATUS_SUBMITTED
    phien_moi = instance_service.running_instance(db, ENTITY, doc.id)
...
def test_tra_lai_qua_bo_may_co_ghi_NHAT_KY_va_ghi_dung_nguoi(db, align):
    ...
    action_service.send_back(db, instance, align["nguoi"]["a"], NGUOI_DUYET, "Sửa lại mục 2", {})
...
def test_duyet_het_cac_buoc_cung_ghi_nhat_ky_dung_nguoi_ky_cuoi(db, align):
    _bat_co(db)
    _luong_hai_buoc(db, align["nguoi"])
    doc = service.submit(db, align["doc"], ACTOR)
    instance = instance_service.running_instance(db, ENTITY, doc.id)

    action_service.approve(db, instance, align["nguoi"]["a"], 77, {})
    action_service.approve(db, instance, align["nguoi"]["b"], 88, {})
...
def test_ban_hanh_sau_khi_bi_tra_lai_bao_dung_ly_do_chu_khong_phai_cau_mu_mo(db, align):
    ...
    doc = service.submit(db, align["doc"], ACTOR)
    instance = instance_service.running_instance(db, ENTITY, doc.id)
    action_service.send_back(db, instance, align["nguoi"]["a"], ACTOR, "Sai thể thức", {})
    db.refresh(doc)
    ...
    assert "Trả lại" in message and "Sai thể thức" in message
    assert "Không có bản nào đang chờ duyệt" not in message


def test_gui_duyet_lai_roi_thi_KHONG_chan_nua(db, align):
    ...
    approval_bridge.block_legacy_path(db, doc)          # không được ném
    assert service.approve(db, doc, ACTOR).status == STATUS_EFFECTIVE
