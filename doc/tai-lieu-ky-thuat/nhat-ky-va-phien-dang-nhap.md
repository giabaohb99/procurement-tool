...
nay **đủ bốn bảng** — bản đầu chỉ gói `audit` + `request`.

**Đợt 2 — `bao-CR-454` (21/09/2026) — XONG, và khác bản vẽ ở ba chỗ.**

Migration `f2c5b9d71a48` chia bốn bảng theo `YEAR(created_at)`, từ năm cũ nhất đang có dữ liệu
tới **năm sau năm nay**, đuôi luôn là `pmax VALUES LESS THAN MAXVALUE`. Helper DDL ở
`modules/system_log/partition.py`; việc dọn 03:50 nay chạy **ba nhịp**: tạo trước phân vùng năm
tới (`maintain_partitions`) · bỏ nguyên phân vùng năm đã quá hạn (`drop_expired_partitions`) ·
rồi mới xóa theo dòng phần tháng còn lại. Hai đường **sống cạnh nhau chứ không thay nhau**: mốc
16 tháng luôn rơi vào GIỮA một năm, nên năm nằm trọn bên ngoài thì bỏ phân vùng, còn mấy tháng
đầu của năm bị mốc cắt đôi thì vẫn phải `DELETE`. Bản sao lưu hằng đêm nay dump **hai lượt**
(`backup/service.py`): lượt 1 `--ignore-table` bốn bảng, lượt 2 `--no-data` đúng bốn bảng đó.

Ba chỗ khác bản vẽ, đều là chỗ bản vẽ nói hụt chứ không phải làm tắt:

1. **Không đòi đủ 12 gói tháng.** `audit.archive` cố ý không đẩy gì lên khi tháng đó rỗng
   (`status: empty`), nên đòi đủ 12 gói theo đúng câu chữ là một năm có một tháng nghỉ thành
   một năm **không bao giờ** bỏ được phân vùng. Luật thật: một tháng coi như đạt khi **đã có gói
   HOẶC hiện không còn dòng nào** dưới DB (`find_missing_archive_months`) — đúng luật mà đường
   `DELETE` vẫn đang dùng.
2. **Thêm chốt thứ ba: không còn dòng nào của năm đó chưa quá hạn** (`count_unexpired_in_year`).
   Ba bảng xét theo `created_at` thì con số này luôn bằng 0; nó tồn tại vì bảng thứ tư —
   `tab_login_session` gom theo lúc MỞ nhưng hết hạn theo lúc ĐÓNG. Hàm đếm bằng **hiệu của hai
   phép đếm**, không bằng `NOT (điều kiện hết hạn)`: điều kiện của phiên so trên hai cột cho
   phép NULL, mà `NOT NULL` trong SQL vẫn ra NULL nên dòng đó rơi khỏi cả hai vế và hàm báo 0
   cho đúng cái dòng nó sinh ra để bắt.
3. **Danh sách bốn bảng ở `backup/service.py` được SUY RA** từ `ARCHIVE_TABLES` chứ không chép
   tay — chép tay thì đúng câu chữ của mục này nhưng dựng lên đúng thứ câu sau của nó cảnh báo.

Phải đổi khóa trước khi chia được: MySQL đòi mọi khóa DUY NHẤT chứa đủ cột trong biểu thức chia,
nên khóa chính cả bốn bảng thành `(id, created_at)`, còn `tab_request_log.request_id` và
`tab_login_session.token_id` thành hai cột với cột định danh đứng trước (chỉ mục vẫn dùng được
cho mọi câu tra theo một cột, không đổi dòng mã nào). **Bỏ và thêm lại khóa chính phải nằm trong
CÙNG một câu `ALTER`** — `id` là AUTO_INCREMENT, tách ra là lỗi 1075 và migration chết giữa
chừng. Model vẫn khai khóa chính **một cột** để `create_all` của pytest dựng được bảng trên
SQLite; mọi thay đổi cấu trúc chỉ nằm trong migration, sau chốt `dialect.name != "mysql"`.
...
| **P6 đợt 2** | **`bao-CR-454` (21/09/2026)** — migration `f2c5b9d71a48` chia bốn bảng theo `YEAR(created_at)` (khóa chính nới thành `(id, created_at)`, hai khóa duy nhất phụ nới theo), helper `modules/system_log/partition.py`, việc dọn 03:50 thành ba nhịp *tạo trước phân vùng năm tới → bỏ phân vùng năm quá hạn → xóa theo dòng phần còn lại*, và **QĐ-C: sao lưu hằng đêm dump hai lượt** (`--ignore-table` cho dữ liệu, `--no-data` giữ cấu trúc). Ba chỗ khác bản vẽ, xem §9 | Xóa năm cũ tức thì, sao lưu đêm nhẹ đi | P6 đợt 1 |
