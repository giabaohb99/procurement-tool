...
| BM-005 | Trung bình | Nhật ký không lưu giá trị trước / sau — không chứng minh được đã đổi gì | **ĐÃ ĐÓNG — vá đủ cả dev lẫn PROD (bao-CR-402 = P4 của bao-CR-312).** `tab_change_log` (migration `d5f7a9c1b3e2`) + `core/change_tracker.py` bám sự kiện ORM, ghi trước/sau **từng cột** cho mọi bảng trừ `NO_LOG_TABLES`. **Prod 14/09/2026** (`main` `68733348`, đẩy trong đợt `5abd5dc3`; migration đã chạy, head prod = `d5f7a9c1b3e2`) |
