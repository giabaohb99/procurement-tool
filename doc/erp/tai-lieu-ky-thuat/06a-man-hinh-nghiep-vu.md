...
| Tạo mới Đơn mua hàng | `/procurement/purchase-orders/new` | `purchase-order-detail-page.tsx` | Dùng chung tệp với màn Chi tiết ĐMH. Form trắng hoặc nhận draft từ YCMH qua state điều hướng (`buildPurchaseOrderLines`). Bảng dòng `PurchaseOrderItemsTable` (dựng trên `LinesTable`). Kiểm trường bắt buộc qua `validatePurchaseOrder` — phải khớp `REQUIRED_LINE_FIELDS` ở `purchase_order/service.py` phía backend. |
...
| Báo cáo khảo sát | `/procurement/survey-report` | `survey-report-page.tsx` | Báo cáo theo DÒNG khảo sát (không phải theo phiếu), gọi `useSurveyReport` → `/api/survey-report`. Lọc theo loại dòng (`kind`: NCC hoặc sản phẩm), kết quả duyệt dòng (`line_approve`), khoảng ngày (`date_from` / `date_to` — **đúng tên này**, gửi `from_date` là FastAPI bỏ qua im lặng) và hộp **Bộ lọc** năm ô phụ ghi URL một lượt lúc bấm *Áp dụng* (CR-247). Bốn ô đếm ở đầu bấm được lọc nhanh; backend tính số đếm trước khi lọc nên bấm qua lại vẫn thấy đủ bốn con số. **Cột: ~55, khai một chỗ ở `REPORT_FIELDS` đầu tệp** — mảng này sinh cả cột bảng lẫn ô CSV, thêm cột là thêm một phần tử; ~17 cột hiện sẵn, phần còn lại `defaultHidden`, CSV luôn xuất đủ (CR-248). Mỗi dòng backend trả về mang **đủ một bộ khoá** cho cả hai loại dòng (loại kia ra chuỗi rỗng / `None`), nên đừng khai khoá nào là tuỳ chọn trong `types/survey-report.ts`. |
...
**4. `required-fields.ts`** (`modules/procurement/utils/required-fields.ts`, CR-107) — nguồn sự thật duy nhất cho trường bắt buộc của YCMH, YCBG và ĐMH: vừa vẽ dấu sao đỏ (`RequiredMark`, `RequiredHeader`), vừa chặn khi gửi duyệt (không chặn khi lưu nháp). Bộ trường ĐMH phải khớp `REQUIRED_LINE_FIELDS` trong `purchase_order/service.py`; YCMH và YCBG chỉ kiểm ở giao diện. VAT cố ý không bắt buộc: giá trị `0` vừa nghĩa "chưa nhập" vừa nghĩa "hàng không chịu thuế".
