...
* **Co giãn độ rộng cột (Column Resizing):** Kéo mép phải cột để co nhỏ hoặc nới rộng tùy ý (minWidth hạ về 40–60px), nháy đúp để tự vừa nội dung (Auto-fit). Từ **bao-CR-359** có thêm nút **_Vừa nội dung_** ngay trên thanh công cụ (trước chỉ nằm trong menu *Cột*, gần như không ai tìm ra) — đây là đường thoát cho mọi cột bị cắt chữ mà sàn khai sẵn không với tới.
...
* **Cột chứa Ô NHẬP phải khai sàn bề rộng *(bao-CR-359)*.** Luật "cấm truncate" ở gạch đầu dòng dưới chỉ với tới chữ do bảng vẽ; chữ nằm TRONG một ô nhập thì chính ô đó tự cắt của mình, và bảng không hay biết. Ô chọn ngày là ca tệ nhất vì cắt xong vẫn đọc ra một ngày hợp lệ: `20/09/2026` và `20/09/2027` cùng hiện `20/09/…`. Khai `control: 'date'` cho MỌI cột có `DatePicker` — `shared/data-table/line-column-width.ts` tự nâng `width` + `minWidth` lên **156px** (đo thật: nút bấm `DatePicker size="sm"` 135,3px + `px-2.5` hai bên của ô bảng 20px). Sàn kẹp **cả bề rộng người dùng đã kéo**, vì `useTableLayout` chỉ ghi vào `columnWidths` những cột người dùng tự chỉnh — sửa `width` trong mã nguồn tới được mọi người trừ đúng người đã kéo cột đó. Trong bảng dòng thì `DatePicker` luôn dùng `size="sm"`; cỡ mặc định (`h-9 px-4`) cần tới ~189px.
...
> URL** `tab_user.avatar`, nay (CR-162) đổi sang **`tab_user.avatar_file_id` → `tab_file`** (bigint,
...
|---|---|---|---|
| NF-20 | ~~**Route chưa khóa theo quyền.** Gõ thẳng URL vào màn không có quyền thì trang vẫn mở, backend trả 403, người dùng thấy trang lỗi thay vì bị chặn tử tế.~~ **XONG (CR-163, 24/08)** — chặn **tập trung ở `ModuleLayout`** thay vì bọc từng route (khỏi sửa 15 module): thêm **`canAccessRoute(module, pathname, can)`** dùng **cùng nguồn luật với menu** *(`visibleNavItems`)* — quyền của màn = quyền của mục thanh bên khớp path DÀI NHẤT, nên trang chi tiết ăn theo quyền mục danh sách; mục con cố ý không gác entity thì vẫn mở. Phân hệ không mở được → đá về màn chọn phân hệ; màn cụ thể ngoài quyền → giữ khung + menu, ruột là **trang 403** (`forbidden-page.tsx`) để chọn màn khác. **Chưa gác** route *in* (`*Print`) — nằm ngoài `ModuleLayout`, backend vẫn chặn dữ liệu | — | Đã làm |
| NF-21 | ~~**Avatar lưu riêng, ảnh cũ mồ côi.** `tab_user.avatar` giữ URL công khai R2; đổi avatar tạo key mới, key cũ không ai xóa → rác R2. Thêm: entity `"avatar"` thừa trong `file_registry.py`.~~ **XONG (CR-162, 24/08)** — đổi cột thành **`avatar_file_id` bigint → `tab_file`** (KHÔNG qua `file_link`, đúng như phán đoán bên phải). Đọc URL qua `@property User.avatar` + `relationship(lazy="selectin")` nên danh sách vẫn nạp gộp không N+1; ghi qua `user/service.set_user_avatar` — **xóa hẳn file cũ** khi đổi. Ảnh **Google** nay **tải về storage của mình** (`sync_google_avatar`), hết URL ngoài → một nguồn duy nhất. Đã **bỏ** entity `"avatar"` thừa. Migration `a3f7c012e9b5` backfill ảnh R2 nội bộ, bỏ qua URL Google *(tự đồng bộ lần login sau)* | — | Đã làm |
