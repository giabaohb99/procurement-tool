...
**Tool trợ lý đã hiểu luồng này (CR-262):** `payable_lookup` đính kèm `prepay_hanging` (kèm hint chỉ đường chính/phụ) khi kết quả gom về đúng một NCC còn treo; `payment_request_read` trả `offset_amount` từng dòng + `offset_total` (`total` chỉ là phần chi thật) + `offset_hint` theo trạng thái phiếu, và với phiếu **Chờ duyệt** còn chạy soát khô `check_line_offsets` (cùng bộ luật với lúc Duyệt, không đụng DB) trả về `offset_check` — trợ lý trả lời được "phiếu này duyệt được chưa / vì sao bị chặn duyệt" và nhắc đúng luật KHÔNG duyệt một phần (mục F.2).

**Tool soạn nháp cũng tự chia cấn trừ (CR-264):** `draft_payment_request` thấy NCC còn treo cấp NCC thì chia sẵn phần cấn trừ theo FIFO hạn nợ (khoản tới hạn sớm ăn trước) trên chính các khoản đã chọn — trả `draft.offsets` + `offset_total` + `cash_total` (phần chi thật), tổng chia không vượt treo còn lại; chỉ gợi ý khi người hỏi có thêm quyền `payment_request.read` (treo là dữ liệu phân hệ YCTT). Nút "Tạo đề nghị thanh toán" dưới câu trả lời truyền tiếp `&offsets=id:tiền,...` trên URL, form tạo phiếu điền sẵn cột *Cấn trừ trả trước* (kẹp không vượt nợ còn lại lúc mở form). Số này chỉ là ĐỀ XUẤT — người dùng sửa/bỏ từng dòng, hệ thống vẫn soát sơ bộ lúc gửi duyệt và soát chặt lúc Duyệt như mô tả ở trên.
