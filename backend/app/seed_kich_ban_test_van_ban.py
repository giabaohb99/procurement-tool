...
def seed_kich_ban_test_van_ban(db) -> int:
    ...
    for ma, ho_ten, ten_phong in DIEN_VIEN:
        phong = (
            db.query(Department)
            .filter(Department.company_id == company.id, Department.name == ten_phong)
            .first()
        )
        if phong is None:
            raise SystemExit(f"Không thấy phòng «{ten_phong}» ở {COMPANY_CODE}.")

        emp = db.query(Employee).filter(Employee.code == ma).first()
        if emp is None:
            emp = Employee(code=ma, full_name=ho_ten, company_id=company.id,
                           department_id=phong.id, position="Nhân viên",
            ...
            emp.department_id = phong.id
            emp.company_id = company.id
            emp.is_active = True

        email = f"{ma.lower()}@dego.test"
        user = db.query(User).filter(User.email == email).first()
        if user is None:
            user = User(email=email, employee_id=emp.id, is_active=True,
                        password_hash=hash_password(ma), created_by=1, updated_by=1)
            db.add(user)
            db.flush()
        else:
            user.password_hash = hash_password(ma)
            user.is_active = True
...
if __name__ == "__main__":
    ...
    with SessionLocal() as db:
        moi = seed_kich_ban_test_van_ban(db)
    print(f"Xong. Thêm mới {moi} nhân sự; toàn bộ {len(DIEN_VIEN)} tài khoản đã đặt lại mật khẩu.")
