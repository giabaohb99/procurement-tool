...
def _heading(type_name: str, subject: str = "") -> str:
    """Khối đầu văn bản HAI CỘT + tên loại — thể thức chung ở `document_the_thuc`."""
    return (layout.opening_block("{{PHAP_NHAN}}", "{{SO_HIEU}}", "Hà Nội, {{NGAY}}")
            + layout.title_block(type_name, subject))


def _recipients_block(*places: str) -> str:
    """Khối «Nơi nhận» — phần không ai được quên ở văn bản gửi ra ngoài."""
    return layout.recipients_block(*places, archive_note="Lưu: VT, Ban Hành chính.")


def _signature_block(job_title: str, full_name: str = "Nguyễn Văn A") -> str:
    """Khối chữ ký: chức vụ, khoảng trống ký tay, họ tên."""
    return layout.single_signature_block(job_title, full_name)


def _table(headers: list[str], *rows: list[str]) -> str:
    """Bảng có hàng tiêu đề — dùng cho biểu mẫu và phụ lục."""
    header_row = "<tr>" + "".join(f"<th><p>{c}</p></th>" for c in headers) + "</tr>"
    body = "".join(
        "<tr>" + "".join(f"<td><p>{c}</p></td>" for c in h) + "</tr>" for h in rows)
    return f"<table><tbody>{header_row}{body}</tbody></table>"


POLICY = _heading("CHÍNH SÁCH QUẢN TRỊ NỘI BỘ") + """
<h3>I. MỤC TIÊU</h3>
...
""" + _recipients_block("Các đơn vị trực thuộc", "Các pháp nhân thành viên",
                "Hội đồng quản trị (để báo cáo)") + _signature_block("TỔNG GIÁM ĐỐC", "Trần Minh Quang")

DECISION = _heading("QUYẾT ĐỊNH", "Về việc ban hành Quy chế công tác văn thư, lưu trữ") + """
<h3>TỔNG GIÁM ĐỐC</h3>
...
""" + _recipients_block("Như Điều 3 (để thực hiện)", "Hội đồng quản trị (để báo cáo)",
                "Các pháp nhân thành viên") + _signature_block("TỔNG GIÁM ĐỐC", "Trần Minh Quang")

OLD_DECISION = _heading("QUYẾT ĐỊNH",
...
""" + _recipients_block("Như Điều 3", "Hội đồng quản trị (để báo cáo)") + _signature_block(
    "TỔNG GIÁM ĐỐC", "Trần Minh Quang")

REGULATION_V1 = _heading("QUY CHẾ", "Công tác văn thư, lưu trữ") + """
...
""" + _signature_block("TỔNG GIÁM ĐỐC", "Trần Minh Quang")

REGULATION_V2 = _heading("QUY CHẾ", "Công tác văn thư, lưu trữ") + """
...
PROCEDURE = _heading("QUY TRÌNH", "Soạn thảo và ban hành văn bản") + """
...
GUIDELINE = _heading("HƯỚNG DẪN", "Thể thức và kỹ thuật trình bày văn bản") + """
...
""" + _signature_block("TL. TỔNG GIÁM ĐỐC<br/>TRƯỞNG BAN HÀNH CHÍNH", "Phạm Thị Bích")

FORM_SIGNATURE_SLIP = _heading("PHIẾU TRÌNH KÝ VĂN BẢN") + """
...
<p>………………………………………………………………………………………</p>
""" + _signature_block("NGƯỜI CÓ THẨM QUYỀN KÝ", "……………………")

FORM_OUTGOING_REGISTER = _heading("SỔ ĐĂNG KÝ VĂN BẢN ĐI") + """
...
""" + _signature_block("TRƯỞNG BAN HÀNH CHÍNH", "Phạm Thị Bích")

CONFIDENTIALITY_RULES = _heading("QUY ĐỊNH", "Về độ mật và thời hạn bảo quản hồ sơ") + """
...
""" + _recipients_block("Các đơn vị trực thuộc", "Các pháp nhân thành viên") + _signature_block(
    "TỔNG GIÁM ĐỐC", "Trần Minh Quang")

ARCHIVE_SUBMISSION_LETTER = _heading("CÔNG VĂN", "V/v hướng dẫn nộp lưu hồ sơ năm 2025") + """
...
""" + _recipients_block("Như trên", "Tổng Giám đốc (để báo cáo)") + _signature_block(
    "TL. TỔNG GIÁM ĐỐC<br/>TRƯỞNG BAN HÀNH CHÍNH", "Phạm Thị Bích")

DUTY_NOTICE = _heading("THÔNG BÁO",
...
DIGITIZATION_PLAN = _heading("KẾ HOẠCH",
...
""" + _recipients_block("Các đơn vị liên quan", "Tổng Giám đốc (để báo cáo)") + _signature_block(
    "TRƯỞNG BAN HÀNH CHÍNH", "Phạm Thị Bích")

PROPOSAL = _heading("TỜ TRÌNH",
...
""" + _recipients_block("Như trên", "Tổng Giám đốc") + _signature_block("TRƯỞNG BAN HÀNH CHÍNH",
                                                   "Phạm Thị Bích")

MEETING_MINUTES = _heading("BIÊN BẢN",
...
""" + _signature_block("CHỦ TỊCH HỘI ĐỒNG", "Phạm Thị Bích")

POWER_OF_ATTORNEY = _heading("GIẤY ỦY QUYỀN", "Về việc ký văn bản hành chính") + """
...
""" + _signature_block("BÊN ỦY QUYỀN<br/>TỔNG GIÁM ĐỐC", "Trần Minh Quang")
