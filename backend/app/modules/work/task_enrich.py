...
Thẻ việc trên kanban cần: người phụ trách, nhãn tùy biến (Tag và Độ ưu tiên nay
cũng là nhãn tùy biến), tiến độ việc con n/m, số bình luận. Hỏi từng thứ trong
vòng lặp là 4 query nhân số thẻ — một bảng 200 việc thành nghìn câu truy vấn.
Tệp này hỏi mỗi thứ đúng một lần cho cả mẻ rồi phát về theo `task_id`.
"""
...
def collect(db: Session, tasks: list[WorkTask]) -> dict:
    ...
    `{assignees, labels, subtasks, comments}` — mỗi khóa là một dict
    ...
    return {"assignees": assignees, "labels": labels,
