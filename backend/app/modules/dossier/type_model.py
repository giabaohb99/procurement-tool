...
⚠️ Bảng HỒ SƠ (`tab_dossier`) **chưa tồn tại** — màn danh sách hồ sơ còn chạy
trên dữ liệu mẫu ở frontend. Khi dựng nó: nối bằng `dossier_type_id` và **vẫn
phải** chép tên loại sang một cột nhãn, vì bản in và tệp Excel đọc thẳng cột chữ
(bài học duoc-CR-320). Kèm theo là hai chốt còn thiếu ở đây — `before_delete`
chặn xóa loại đang có hồ sơ dùng, và `before_update` chép tên mới sang các hồ sơ
đang mang tên cũ.
"""
