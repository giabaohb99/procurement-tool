...
def target_status(db, pr: PurchaseRequest) -> str | None:
    ...
    if not items:
        return None
    # B-06: erp-v2 lưu MÃ line_status — bản main so chuỗi tiếng Việt.
    active = [i for i in items if (i.line_status or LINE_STATUS_NO_PO) != LINE_STATUS_CANCELLED]
    if active and all((i.line_status or "") == LINE_STATUS_COMPLETED for i in active):
