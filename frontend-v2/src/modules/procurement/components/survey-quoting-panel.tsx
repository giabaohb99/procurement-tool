...
//  Ô chọn "Tiến độ dòng" chỉ bày các nhãn CÒN GẶP được trong bước So giá — dòng đã
//  tạo YCMH / lên đơn / hoàn thành đã bị `phase=quoting` loại từ server, bày nhãn
//  của chúng ra là mời người dùng chọn một bảng rỗng.
const QUOTING_STATES = [
  'Chưa tiếp nhận',
  'Đã tiếp nhận',
  'Đang khảo sát',
  'Đã trả kết quả',
...
const DATE_FIELDS = [
  {
    value: 'received',
    label: 'Theo ngày tiếp nhận',
    from: 'received_date_from',
    to: 'received_date_to',
  },
  {
    value: 'result_due',
    label: 'Theo hạn trả KQ',
    from: 'result_due_date_from',
    to: 'result_due_date_to',
  },
...
const FILTER_CONFIG = {
  ...
  //  nâng cao xong mất luôn ô đó. `step` là của trang gộp bên ngoài — mất nó là
  //  văng khỏi bước So giá về bảng ĐMH.
  preserveParams: [
    'step',
    'state',
    'late',
...
/**
 * P6-6 (bao-CR-284) — bước "Đang so giá" của màn Tiến độ mua hàng gộp.
 *
 * Là ruột màn "Tiến độ báo giá" cũ chuyển thành panel: bảng DÒNG yêu cầu báo giá
 * chưa rời giai đoạn báo giá (server lọc `phase=quoting` — chưa hoàn thành, chưa
 * tạo YCMH, chưa lên đơn thẳng). Quyền dữ liệu / che cột NCC / xuất Excel giữ
 * nguyên luật của `/api/survey-progress` (CR-075).
 */
export function SurveyQuotingPanel() {
  return (
    <FilterProvider config={FILTER_CONFIG}>
      <SurveyQuotingContent />
    </FilterProvider>
  )
}

function SurveyQuotingContent() {
  ...
  const handleExportExcel = async () => {
    ...
    await downloadFile(
      `/api/survey-progress/export/xlsx?${query.toString()}`,
      'tien-do-so-gia.xlsx',
    )
  }
  ...
  const columns = useMemo<DataTableColumn<SurveyProgressItem>[]>(() => {
    const allCols: (DataTableColumn<SurveyProgressItem> & { supplierOnly?: boolean })[] = [
      ...
      {
        key: 'internal_line_code',
        header: 'Mã dòng',
        width: 120,
        defaultHidden: true,
        supplierOnly: true,
        wrap: true,
        cell: (r) => r.internal_line_code || '',
      },
      {
        key: 'item_group',
        header: 'Phân loại',
      ...
      },
      {
        ...
        width: 100,
        align: 'right',
        cell: (r) => <span className="tabular-nums">{formatQuantity(r.request_qty) || 0}</span>,
      },
      { key: 'uom', header: 'ĐVT', width: 70, cell: (r) => r.uom || '' },
      {
        ...
        align: 'right',
        defaultHidden: true,
        cell: (r) => <span className="tabular-nums">{formatUnitPrice(r.proposed_price) || 0}</span>,
      },
      {
        key: 'assignee_name',
      ...
      },
      {
        ...
        header: 'Hạn trả KQ',
        width: 110,
        cell: (r) => formatDate(r.result_due_date) || '',
      },
      {
        key: 'result_date',
        header: 'Ngày trả KQ',
        width: 110,
        cell: (r) => formatDate(r.result_date) || '',
      },
      {
        key: 'days_late',
        header: 'Trễ (ngày)',
        ...
        cell: (r) =>
          r.days_late && r.days_late > 0 ? (
            <span className="font-semibold text-destructive">{r.days_late}</span>
          ) : (
            ''
          ),
      },
      {
        ...
        width: 110,
        align: 'right',
        cell: (r) => r.handling_days ?? '',
      },
      {
        key: 'progress_state',
        header: 'Tiến độ dòng',
        width: 160,
        cell: (r) => <ProgressStateBadge state={r.progress_state} />,
      },
      {
        key: 'line_status',
        header: 'TT dòng',
      ...
      },
      {
        ...
        width: 80,
        align: 'right',
        cell: (r) => r.option_count ?? 0,
      },
      {
        key: 'opt_label',
        header: 'Phương án chốt',
        width: 130,
        defaultHidden: true,
        wrap: true,
        cell: (r) => r.opt_label || '',
      ...
      {
        key: 'opt_supplier_code',
        header: 'Mã NCC',
        width: 120,
        defaultHidden: true,
        supplierOnly: true,
        wrap: true,
        cell: (r) => r.opt_supplier_code || '',
      },
      {
        key: 'opt_supplier_name',
        header: 'Nhà cung cấp',
        width: 220,
        supplierOnly: true,
        wrap: true,
        cell: (r) => r.opt_supplier_name || '',
      },
      {
        key: 'opt_internal_code',
        header: 'Mã SP theo NCC',
        width: 140,
        defaultHidden: true,
        supplierOnly: true,
        wrap: true,
        cell: (r) => r.opt_internal_code || '',
      },
      {
        key: 'opt_product_code',
        header: 'Mã SP hệ thống',
        width: 140,
        defaultHidden: true,
        wrap: true,
        cell: (r) => r.opt_product_code || '',
      },
      {
        key: 'opt_product_name',
        header: 'Tên SP báo giá',
        width: 220,
        wrap: true,
        cell: (r) => <span className="font-medium">{r.opt_product_name || ''}</span>,
      },
      {
        key: 'opt_spec',
        header: 'Quy cách',
      ...
      },
      {
        ...
        align: 'right',
        defaultHidden: true,
        cell: (r) => <span className="tabular-nums">{formatQuantity(r.opt_moq) || 0}</span>,
      },
      {
        key: 'opt_price',
        header: 'Đơn giá báo',
        ...
        align: 'right',
        cell: (r) => (
          <span className="font-semibold tabular-nums">{formatUnitPrice(r.opt_price) || 0}</span>
        ),
      },
      {
        key: 'opt_volume_range',
        header: 'Khoảng SL áp giá',
        width: 140,
        defaultHidden: true,
        wrap: true,
        cell: (r) => r.opt_volume_range || '',
      },
      {
        key: 'opt_vat',
        header: 'VAT%',
      ...
      {
        key: 'opt_delivery_time',
        header: 'Thời gian giao',
        width: 130,
        defaultHidden: true,
        wrap: true,
        cell: (r) => r.opt_delivery_time || '',
      },
      {
        key: 'opt_delivery_place',
        header: 'Nơi giao',
        width: 160,
        defaultHidden: true,
        wrap: true,
        cell: (r) => r.opt_delivery_place || '',
      },
      {
        key: 'opt_shipping_cost',
        header: 'Phí vận chuyển',
        width: 120,
        align: 'right',
        defaultHidden: true,
        cell: (r) => <span className="tabular-nums">{formatMoney(r.opt_shipping_cost) || 0}</span>,
      },
      {
        key: 'opt_sample_ready',
        header: 'Có mẫu',
        width: 80,
        defaultHidden: true,
        cell: (r) => (r.opt_sample_ready ? 'Có' : ''),
      },
      {
        key: 'opt_lab_result',
        header: 'KQ kiểm nghiệm',
        width: 130,
        defaultHidden: true,
        wrap: true,
        cell: (r) => r.opt_lab_result || '',
      },
      {
        key: 'opt_note',
        header: 'Ghi chú NSTM',
  ...
  return (
    <DataTable
      ...
      isLoading={isLoading}
      isError={isError}
      emptyMessage="Không có dòng yêu cầu báo giá nào đang so giá."
      ...
      toolbar={
        <>
          ...
          {/*  Nút xuất nằm trong panel (không trên PageHeader của trang gộp) vì nó
               xuất TẬP CỦA BƯỚC NÀY qua `/api/survey-progress`, quyền riêng
               `survey_request.export` — khác hẳn nút xuất của bảng ĐMH. */}
          {canExport && (
