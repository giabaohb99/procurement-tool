...
import { parseMoney } from './doc-tien'

describe('docTien — đọc số tiền thành chữ', () => {
  it('trả "Không đồng" cho số 0 và số âm, không bao giờ đọc số âm', () => {
    expect(parseMoney(0)).toBe('Không đồng')
    expect(parseMoney(-5000)).toBe('Không đồng')
  })

  it('đọc tròn triệu, không chèn "lẻ" thừa', () => {
    expect(parseMoney(1_000_000)).toBe('Một triệu đồng chẵn')
  })

  it('chèn "lẻ" khi hàng chục bằng 0 nhưng hàng đơn vị có chữ', () => {
    expect(parseMoney(105_000)).toBe('Một trăm lẻ năm nghìn đồng chẵn')
  })

  it('đọc "mốt" ở hàng đơn vị sau "mươi"', () => {
    expect(parseMoney(21_000_000)).toBe('Hai mươi mốt triệu đồng chẵn')
  })

  it('đọc "mười … lăm" cho khoảng 11-19 kết thúc bằng 5', () => {
    expect(parseMoney(15_000)).toBe('Mười lăm nghìn đồng chẵn')
  })

  it('làm tròn tới đồng trước khi đọc, không để phần lẻ rơi vào chữ', () => {
    // 1500.6 -> làm tròn 1501; "lẻ" nằm trong nhóm hàng đơn vị.
    expect(parseMoney(1500.6)).toBe('Một nghìn năm trăm lẻ một đồng chẵn')
  })

  it('đọc đủ bốn cấp tỷ / triệu / nghìn / trăm cho số lớn', () => {
    expect(parseMoney(1_234_567_890)).toBe(
