...
function makeActivity(id: number, at: string, message = 'Tạo công việc: A'): WorkActivity {
  return {
    id,
    kind: 1,
    action: 'create',
...
describe('groupActivitiesByDay', () => {
  it('reads the timestamp as UTC even without a Z, so 01:00 local stays today', () => {
    ...
    //  rơi nhầm về "Hôm qua".
    const days = groupActivitiesByDay([makeActivity(1, '2026-08-30T18:30:00')], NOW)
  ...
  it('labels the previous calendar day as Hôm qua and older days as a date', () => {
    const days = groupActivitiesByDay(
      [makeActivity(3, '2026-08-31T02:00:00'), makeActivity(2, '2026-08-30T02:00:00'), makeActivity(1, '2026-08-05T02:00:00')],
      NOW,
    )
  ...
  it('keeps one group per day even when the day is split across two fetched pages', () => {
    ...
    const page1 = [makeActivity(5, '2026-08-31T02:00:00'), makeActivity(4, '2026-08-31T01:00:00')]
    const page2 = [makeActivity(3, '2026-08-31T00:30:00'), makeActivity(2, '2026-08-30T02:00:00')]
    const days = groupActivitiesByDay([...page1, ...page2], NOW)
  ...
  it('preserves the newest-first order the backend sent, without re-sorting', () => {
    const days = groupActivitiesByDay(
      [makeActivity(9, '2026-08-31T05:00:00'), makeActivity(8, '2026-08-31T04:00:00')],
      NOW,
    )
  ...
  it('shows rows with a broken timestamp instead of swallowing them', () => {
    ...
    const days = groupActivitiesByDay([makeActivity(1, ''), makeActivity(2, 'khong-phai-ngay')], NOW)
  ...
  it('adds the year to the month label when the day is not in the current year', () => {
    ...
    const days = groupActivitiesByDay([makeActivity(1, '2025-08-05T02:00:00')], NOW)
  ...
  it('does not merge two different days that share a day-of-month', () => {
    ...
    const days = groupActivitiesByDay(
      [makeActivity(2, '2026-08-31T02:00:00'), makeActivity(1, '2026-07-31T02:00:00')],
      NOW,
    )
    expect(days).toHaveLength(2)
  })
