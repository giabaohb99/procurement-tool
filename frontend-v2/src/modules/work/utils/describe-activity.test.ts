...
function makeActivity(over: Partial<WorkActivity> = {}): WorkActivity {
  return {
    id: 1,
    kind: 1,
    action: 'create',
...
describe('describeActivity', () => {
  it('drops the trailing task name, which the row already shows on its own line', () => {
    expect(describeActivity(makeActivity())).toBe('đã tạo công việc')
  })
  ...
  it('leaves the sentence whole when the row has no task name to show', () => {
    expect(describeActivity(makeActivity({ task_title: '' }))).toBe('đã tạo công việc: Dựng khung')
  })

  it('does not strip a task name that merely appears in the middle', () => {
    const activity = makeActivity({ message: 'Sửa công việc: Dựng khung xong', task_title: 'Dựng khung' })
  ...
  it('keeps Vietnamese diacritics when lowering the first letter', () => {
    expect(describeActivity(makeActivity({ message: 'Đổi người phụ trách' }))).toBe(
      'đã đổi người phụ trách',
    )
  })

  it('leaves an all-caps abbreviation alone instead of breaking it', () => {
    expect(describeActivity(makeActivity({ message: 'YCMH đã được duyệt' }))).toBe(
  ...
  it('falls back to the action label when the log line has no message', () => {
    ...
    expect(describeActivity(makeActivity({ message: '' }))).toBe('đã tạo mới')
    expect(describeActivity(makeActivity({ message: '   ' }))).toBe('đã tạo mới')
  })

  it('returns an empty clause when there is nothing at all to say', () => {
    expect(describeActivity(makeActivity({ message: '', action_label: '' }))).toBe('')
  })

  it('does not touch a sentence that already starts lowercase or with a digit', () => {
    expect(describeActivity(makeActivity({ message: 'gỡ nhân sự #4' }))).toBe('đã gỡ nhân sự #4')
    expect(describeActivity(makeActivity({ message: '3 việc bị dời hạn' }))).toBe('đã 3 việc bị dời hạn')
  })
})
