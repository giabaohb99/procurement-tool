...
import { WORK_ROLE, type WorkMember } from '../types/work'
...
interface MembersPanelProps {
  open: boolean
  members: WorkMember[]
  /** Vai trò của CHÍNH mình — quyết định hiện hàng mời / nút gỡ (04 §3). */
  myRole: number | null
  /** Chữ trong câu rỗng: «dự án này» / «nhóm này». */
  subject: string
  isInviting: boolean
  onInvite: (employeeId: number, role: number) => void
  onChangeRole: (employeeId: number, role: number) => void
  onRemove: (memberId: number) => void
}

/**
 * Khối THÀNH VIÊN dùng chung cho DỰ ÁN lẫn NHÓM (bao-CR-482): mời · đổi vai trò ·
 * gỡ. Phần thân thuần hiển thị — dữ liệu và hook đi từ ngoài vào (`ListMembersPanel`,
 * `GroupMembersPanel`), vì hai cửa API khác nhau nhưng khuôn màn giống hệt.
 *
...
export function MembersPanel({
  ...
  function invite() {
    picked.forEach((employeeId) => onInvite(employeeId, inviteRole))
    setPicked([])
  }
    ...
    <section className="space-y-3">
      ...
      {canManage && canReadDirectory && (
        ...
        <div className="flex items-end gap-2 rounded-lg border bg-muted/30 p-2.5">
          ...
          />
          <Button onClick={invite} disabled={picked.length === 0 || isInviting}>
      ...
      {/*  KHÔNG tự cuộn trong khung. Cả hộp thoại đã cuộn được (overlay lo), và
           ...
           (03/09/2026). Đông người thì hộp dài ra và cuộn một mạch — cũng đúng
           hơn: hàng mời nằm ngay trên, không bị kẹt ngoài vùng cuộn. */}
      <ul className="divide-y rounded-lg border">
        ...
        {rows.length === 0 && (
          <li className="px-3 py-6 text-center text-sm text-muted-foreground">
            {members.length === 0
              ? `Chưa có ai được mời riêng vào ${subject}.`
