 ...
 * chỗ vừa bị gỡ còn thứ tự mới thì chưa về (cập nhật lạc quan của TanStack
...
export interface TaskGroupsBoardProps extends TaskRowActions {
  groups: TaskGroup[]
  ...
  /**
   * Việc đang bung việc con — ĐIỀU KHIỂN TỪ NGOÀI, không giấu trong nhóm.
   *
   * Khung nhìn Gantt phải biết chính xác đang có bao nhiêu dòng để vẽ đúng bấy
   * nhiêu hàng bên trục thời gian; state nằm bên trong thì hai bên lệch hàng
   * ngay khi ai đó bung một việc con.
   */
  expandedTaskId: number | null
  onToggleExpand: (taskId: number) => void
  /**
   * Chiều cao CỐ ĐỊNH của mỗi dòng (px). Chỉ Gantt truyền: bên đó mỗi dòng của
   * lưới trái phải cao đúng bằng một hàng của trục thời gian, lệch một pixel là
   * cuối bảng thanh trượt hẳn khỏi tên việc. Khung nhìn Danh sách để trống cho
   ...
   *
   * Chỉ Gantt cần: lúc kéo cột, nhóm THU LẠI còn mỗi dải tiêu đề, nên trục thời
   * gian phải thu đúng nhóm ấy theo — không thì bên trái mất mấy dòng còn bên
   * phải giữ nguyên, và mọi thanh bên dưới trượt khỏi tên việc của nó suốt cú
   * kéo.
   */
  onDraggingSectionChange?: (sectionId: number | null) => void
...
/**
 * Cụm NHÓM VIỆC kéo thả được — ruột dùng chung của khung nhìn **Danh sách** và
 * lưới trái của khung nhìn **Gantt**.
 *
 * Tách ra khỏi `task-list-view.tsx` khi Gantt cần đúng bộ thao tác này (kéo
 * việc / việc con / cột, bung việc con, dòng «Việc mới»): chép sang bản thứ hai
 * thì hai bên lệch nhau ngay ở lần sửa kế tiếp, mà lệch ở đây là **kéo thả sai
 * chỗ** chứ không phải sai màu sắc.
 *
...
export function TaskGroupsBoard({
  ...
  function handleDragStart(event: DragStartEvent) {
    ...
    if (kind === 'column' && typeof data?.sectionId === 'number') {
      onDraggingSectionChange?.(data.sectionId)
    }
  }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const task = groups.flatMap((g) => g.tasks).find((t) => t.id === activeId)
  ...
  return (
    ...
    >
      ...
      <div className={cn(drag?.blocked && '[&_*]:cursor-not-allowed')}>
