...
function makeAssignee(id: number, name: string, kind: number = WORK_ASSIGNEE_KIND.PIC): WorkAssignee {
  return { employee_id: id, kind, employee_name: name, employee_code: `NV${id}` }
}

function makeMember(id: number, name: string): WorkMember {
  return {
    id,
    employee_id: id,
...
const ALL_MEMBERS = [
  makeMember(1, 'Trần Minh Được'),
  makeMember(2, 'Mộc'),
  makeMember(3, 'Vũ Kinh Doanh'),
]

function renderPicker(assignees: WorkAssignee[], disabled = false) {
  render(
    <TaskAssigneePicker
      assignees={assignees}
      members={ALL_MEMBERS}
...
describe('TaskAssigneePicker — dáng đầy đủ (panel chi tiết)', () => {
  it('MỘT người thì hiện TÊN họ, không phải «1 người phụ trách»', () => {
    ...
    renderPicker([makeAssignee(1, 'Trần Minh Được')])
    expect(screen.getByText('Trần Minh Được')).toBeInTheDocument()
  })

  it('NHIỀU người thì gom thành «N người phụ trách»', () => {
    renderPicker([makeAssignee(1, 'Trần Minh Được'), makeAssignee(2, 'Mộc')])
  ...
  it('KHÔNG đếm người theo dõi vào con số phụ trách', () => {
    ...
    renderPicker([makeAssignee(1, 'Trần Minh Được'), makeAssignee(2, 'Mộc', WORK_ASSIGNEE_KIND.FOLLOWER)])
    expect(screen.getByText('Trần Minh Được')).toBeInTheDocument()
  ...
  it('CHỈ XEM mà có người thì vẫn đọc được, vẫn KHÔNG có nút', () => {
    renderPicker([makeAssignee(1, 'Trần Minh Được'), makeAssignee(2, 'Mộc')], true)
    expect(screen.getByText('2 người phụ trách')).toBeInTheDocument()
    expect(screen.queryByRole('button')).not.toBeInTheDocument()
  })
