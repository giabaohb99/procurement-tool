...
const ENTITY = 'seal_request'

/**
 * Bảng LUỒNG DUYỆT NHIỀU BƯỚC của một phiếu đóng dấu — chỉ hiện khi phiếu thật sự
 ...
 */
export function SealApprovalPanel({ requestId }: { requestId: number }) {
  const { data: instance } = useQuery({
    queryKey: queryKeys.approval.ofEntity(ENTITY, requestId),
    queryFn: () => approvalApi.ofEntity(ENTITY, requestId),
    enabled: requestId > 0,
  })
  ...
  const myTask = (tasks?.items ?? []).find(
    (t) => t.entity === ENTITY && t.entity_id === requestId,
  )
