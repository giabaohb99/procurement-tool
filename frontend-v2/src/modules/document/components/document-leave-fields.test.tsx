...
function soODangHien() {
  return screen.getAllByText('Buổi').length
}

function datNgay(form: UseFormReturn<DocumentRecordFormValues>, tu: string, den?: string) {
  act(() => {
...
describe('DocumentLeaveFields', () => {
  ...
  it('nghỉ trong MỘT ngày thì gộp còn một ô buổi', () => {
    ...
    const lay = dung()
    datNgay(lay(), '2026-09-11', '2026-09-11')

    expect(soODangHien()).toBe(1)
  })

  it('nghỉ nhiều ngày thì vẫn đủ hai ô buổi', () => {
    const lay = dung()
    datNgay(lay(), '2026-09-11', '2026-09-14')
