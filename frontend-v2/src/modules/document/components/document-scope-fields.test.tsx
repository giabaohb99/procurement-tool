...
vi.mock('@/modules/hr/hooks/use-companies', () => ({
  useCompanies: () => ({ data: { items: COMPANIES } }),
}))
...
vi.mock('@/modules/hr/hooks/use-departments', () => ({
  ...
  useDepartmentsByCompanies: () => ({ data: DEPARTMENT_LEVEL }),
}))
...
describe('DocumentScopeFields', () => {
  ...
  it('thêm lại đúng pháp nhân đã có thì không đẻ dòng trùng', async () => {
    ...
    expect(screen.getAllByText(COMPANIES[0].name)).toHaveLength(1)
  })
  ...
  it('chiều PHÒNG BAN: chọn nhiều pháp nhân rồi chọn nhiều phòng, mỗi cặp một dòng', async () => {
    ...
    expect(
      screen.getByText(`Phòng Kế toán — ${COMPANIES[0].name}`),
    ).toBeInTheDocument()
    expect(
      screen.getByText(`Phòng Kế toán — ${COMPANIES[1].name}`),
    ).toBeInTheDocument()
  })
  ...
  it('pháp nhân chưa khai phòng ban nào thì GỌI TÊN nó, không bỏ mặc ô trống', async () => {
    ...
    const warning = screen.getByText(/chưa khai\s+phòng ban nào nên không có gì để chọn/)
    expect(warning).toHaveTextContent(COMPANIES[2].name)
