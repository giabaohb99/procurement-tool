...
describe('soBanKeTiep', () => {
  it('sửa lớn thì lên đầu số kế tiếp và trả phần lẻ về 0', () => {
    expect(nextVersionNo([ban(1, 0)], CHANGE_KIND.major)).toBe('2.0')
    expect(nextVersionNo([ban(2, 3)], CHANGE_KIND.major)).toBe('3.0')
  })

  it('sửa nhỏ thì giữ đầu số, cộng phần lẻ', () => {
    expect(nextVersionNo([ban(1, 0)], CHANGE_KIND.minor)).toBe('1.1')
    expect(nextVersionNo([ban(2, 3)], CHANGE_KIND.minor)).toBe('2.4')
  })
  ...
  it('không còn nói bản 1.1 khi văn bản đã đi qua bản 1', () => {
    expect(nextVersionNo([ban(2, 1)], CHANGE_KIND.minor)).not.toBe('1.1')
  })

  it('đếm từ số CAO NHẤT, không phải bản cuối danh sách', () => {
    expect(nextVersionNo([ban(2, 0), ban(1, 0), ban(1, 1)], CHANGE_KIND.major)).toBe('3.0')
    //  10.0 phải lớn hơn 2.0 — đúng lý do `major`/`minor` là hai số nguyên chứ
    //  không phải chuỗi "2.0".
    expect(nextVersionNo([ban(2, 0), ban(10, 0)], CHANGE_KIND.minor)).toBe('10.1')
  })
