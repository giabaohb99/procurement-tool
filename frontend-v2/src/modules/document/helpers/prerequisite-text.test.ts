...
function dong(overrides: Partial<DocPrerequisite> = {}): DocPrerequisite {
  return {
    sort_order: 1,
    relation: 4,
...
describe('prerequisiteText', () => {
  it('nói thẳng "chưa có văn bản nào" khi kho trống', () => {
    expect(prerequisiteText(dong())).toBe('Chưa có văn bản nào còn hiệu lực')
  })
  ...
  it('nói rõ có mấy cái khi kho có nhưng chưa đủ số lượng', () => {
    ...
    expect(prerequisiteText(dong({ need: 2, available: 1 }))).toBe(
