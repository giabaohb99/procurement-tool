...
export interface ConversationListProps {
  ...
  /** Nút phụ ở cuối hàng đầu — cột bên trái dùng để đặt nút thu gọn. */
  trailing?: ReactNode
}

/**
 * RUỘT của cột hội thoại: hàng «Hội thoại mới» + danh sách đã lưu.
 *
 * Tách khỏi `ConversationSidebar` vì cùng một danh sách này phải dựng ở HAI
 * nơi: cột trái cố định ở màn rộng, và tờ trượt ở khổ điện thoại (xem
 * `ConversationSheet`). Để hai bản chép thì mọi thứ sửa về sau — cách sắp mục,
 * câu xác nhận xóa, chỗ nhét mốc thời gian — phải nhớ sửa cả hai, và bản quên
 * sửa sẽ im lặng lệch đi.
 *
 * Ba điều dựng theo mẫu khách đưa (26/08/2026), đều để **nhìn được nhiều hội
 * thoại hơn trong một tầm mắt**:
 *
 *  - **Mỗi hội thoại một DÒNG**, chỉ có tiêu đề. Bản trước in thêm mốc thời gian
 *    xuống dòng dưới, tức mỗi mục chiếm gấp đôi chỗ để nói một thứ người dùng
 ...
 *    màn này thứ đáng nổi bật nhất là ô nhập câu hỏi.
